...
term.update = function () {
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
        const bullet = bullets[i];
        term.drawString(bullet.x, bullet.y, '*', wglt.Colors.YELLOW);
        bullet.y -= 2;
        if (bullet.y < 0) {
            bullets.splice(i, 1);
        }
    ...
    for (let i = explosions.length - 1; i >= 0; i--) {
        ...
        term.drawString(Math.round(explosion.x), Math.round(explosion.y), '%', wglt.Colors.ORANGE);
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        const enemy = enemies[i];
        term.drawString(enemy.x, enemy.y, '#', wglt.Colors.LIGHT_GREEN);
        term.drawString(enemy.x - 1, enemy.y - 1, '@', wglt.Colors.LIGHT_RED);
        term.drawString(enemy.x + 1, enemy.y - 1, '@', wglt.Colors.LIGHT_RED);
        term.drawString(enemy.x - 1, enemy.y + 1, '@', wglt.Colors.LIGHT_RED);
        term.drawString(enemy.x + 1, enemy.y + 1, '@', wglt.Colors.LIGHT_RED);
    }

    term.drawString(player.x, player.y, '^', wglt.Colors.LIGHT_GRAY);
    term.drawString(player.x - 1, player.y + 1, '/', wglt.Colors.LIGHT_GRAY);
    term.drawString(player.x + 1, player.y + 1, '\\', wglt.Colors.LIGHT_GRAY);
    term.drawString(player.x, player.y + 1, '|', wglt.Colors.LIGHT_RED);

    if (waveTime < 0 && waveTime > -WAVE_WARMUP_TIME + 60) {
        term.drawCenteredString(WIDTH / 2, HEIGHT / 2, 'WAVE ' + wave.count, wglt.Colors.WHITE);
    }

    term.drawString(1, HEIGHT - 2, 'Z - SHOOT', wglt.Colors.WHITE);
