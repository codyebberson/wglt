...
class Item {
    ...
    constructor(useFunction: (item: Item) => void) {
...
class Equipment {
    constructor(slot, powerBonus, defenseBonus, maxHpBonus) {
        this.slot = slot;
    ...
    equip() {
        ...
        this.equipped = true;
        addMessage('Equipped ' + this.owner.name + ' on ' + this.slot + '.', Colors.LIGHT_GREEN);
    }
    ...
    dequip() {
        ...
        this.equipped = false;
        addMessage('Dequipped ' + this.owner.name + ' on ' + this.slot + '.', Colors.YELLOW);
    }
}
...
function placeObjects(room: Rect) {
    ...
    for (let i = 0; i < numItems; i++) {
        ...
        if (choice === 'heal') {
            ...
            // Create a sword
            item = new Entity(x, y, '/', 'sword', Colors.LIGHT_CYAN, false, {
            ...
            // Create a shield
            item = new Entity(x, y, '[', 'shield', Colors.BROWN, false, {
...
function renderBar(x: number, y: number, totalWidth: number, name: string, value: number, maximum: number, barColor: Color, backColor: Color) {
...
function addMessage(msg: string, opt_color?: Color) {
...
function useInventory(choice: number) {
    if (choice >= 0) {
        (inventory[choice].item as Item).use();
    }
}
...
function castLightning(item: Item) {
    ...
    (monster.fighter as Fighter).takeDamage(LIGHTNING_DAMAGE);
    item.remove();
}

function castFireball(item: Item) {
...
function castConfuse(item: Item) {
    ...
    startTargeting((x, y) => {
        ...
        monster.ai = new ConfusedMonster(monster.ai as AI);
        (monster.ai as AI).owner = monster;
        addMessage('The eyes of the ' + monster.name + ' look vacant, as he stumbles around!', Colors.LIGHT_GREEN);
        item.remove();
    });
}

function startTargeting(callback: (x: number, y: number) => void) {
    targetFunction = callback;
    targetCursor.x = player.x;
    targetCursor.y = player.y;
}

function endTargeting(x: number, y: number) {
    if (targetFunction) {
        targetFunction(x, y);
        cancelTargeting();
    }
...
function renderAll() {
    ...
    renderBar(
        ...
        'HP', (player.fighter as Fighter).hp, (player.fighter as Fighter).maxHp,
    ...
    if (targetFunction) {
        const targetCell = term.getCell(targetCursor.x, targetCursor.y);
        if (targetCell) {
...
function newGame() {
    ...
    const dagger = new Entity(0, 0, '-', 'dagger', Colors.LIGHT_CYAN, false, {
