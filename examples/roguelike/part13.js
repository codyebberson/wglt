...
var Fighter = /** @class */ (function () {
    ...
    Object.defineProperty(Fighter.prototype, "maxHp", {
        get: function () {
            return this.baseMaxHp + getAllEquipped(this.owner).reduce(function (acc, item) { return acc + item.maxHpBonus; }, 0);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Fighter.prototype, "defense", {
        get: function () {
            return this.baseDefense + getAllEquipped(this.owner).reduce(function (acc, item) { return acc + item.defenseBonus; }, 0);
        },
        enumerable: false,
        configurable: true
    });
    Object.defineProperty(Fighter.prototype, "power", {
        get: function () {
            return this.basePower + getAllEquipped(this.owner).reduce(function (acc, item) { return acc + item.powerBonus; }, 0);
        },
        enumerable: false,
        configurable: true
    });
...
var Equipment = /** @class */ (function () {
    ...
    Equipment.prototype.toggleEquip = function () {
        if (this.equipped) {
            this.dequip();
        }
        else {
    ...
    Equipment.prototype.equip = function () {
        ...
        var old = getEquippedInSlot(this.slot);
        if (old) {
            old.dequip();
        }
        // Equip object and show a message about it
        this.equipped = true;
        addMessage('Equipped ' + this.owner.name + ' on ' + this.slot + '.', colors_1.Colors.LIGHT_GREEN);
    };
    Equipment.prototype.dequip = function () {
        ...
        this.equipped = false;
        addMessage('Dequipped ' + this.owner.name + ' on ' + this.slot + '.', colors_1.Colors.YELLOW);
    };
    return Equipment;
}());
...
function getAllEquipped(obj) {
    if (obj === player) {
        return inventory.filter(function (item) { return item.equipment && item.equipment.equipped; }).map(function (item) { return item.equipment; });
    }
    else {
        return [];
    }
...
function placeObjects(room) {
    ...
    for (var i = 0; i < numItems; i++) {
        ...
        if (choice === 'heal') {
            ...
            // Create a sword
            item = new Entity(x, y, '/', 'sword', colors_1.Colors.LIGHT_CYAN, false, {
            ...
            // Create a shield
            item = new Entity(x, y, '[', 'shield', colors_1.Colors.BROWN, false, {
...
function handleKeys() {
    ...
    if (term.isKeyPressed(keys_1.Keys.VK_I)) {
        if (inventory.length === 0) {
            ...
            var options = inventory.map(function (item) {
...
function newGame() {
    ...
    var dagger = new Entity(0, 0, '-', 'dagger', colors_1.Colors.LIGHT_CYAN, false, {
...
var term = new terminal_1.Terminal(document.querySelector('canvas'), SCREEN_WIDTH, SCREEN_HEIGHT, { frameDelay: 5 });
