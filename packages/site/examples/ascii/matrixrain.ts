import { Cell, Terminal } from '@wglt/ascii';
import { CgaPalette, RNG, fromRgb } from '@wglt/core';
...
const term = new Terminal('canvas', w, h);
...
term.update = () => {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const color = rain[y][x] === 255 ? CgaPalette.WHITE : fromRgb(0, rain[y][x], 0);
