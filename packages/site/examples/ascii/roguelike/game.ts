import { Cell, Console, MessageDialog, SelectDialog, computePath } from '@wglt/ascii';
import { CgaPalette, Color, Key, RNG, Rect, fromRgb } from '@wglt/core';
...
export class Game implements AppState {
  ...
  constructor(app: App) {
    ...
    this.addMessage('Welcome stranger! Prepare to perish!', CgaPalette.DARK_RED);

    // Initial equipment: a dagger
    const dagger = new Item(this, 0, 0, '-', 'Dagger', CgaPalette.LIGHT_CYAN);
  ...
  createMap(): Console {
    ...
    this.stairs = new Entity(this, stairsLoc.x, stairsLoc.y, '<', 'Stairs', CgaPalette.WHITE);
  ...
  placeObjects(room: Rect): void {
    ...
    for (let i = 0; i < numMonsters; i++) {
      ...
      if (choice === 'orc') {
        monster = new Actor(this, x, y, 'o', 'Orc', CgaPalette.LIGHT_GREEN);
        monster.hp = 20;
        ...
        monster = new Actor(this, x, y, 'T', 'Troll', CgaPalette.DARK_GREEN);
        monster.hp = 30;
  ...
  addMessage(msg: string, opt_color?: Color): void {
    ...
    this.messages.push({ text: msg, color: opt_color || CgaPalette.WHITE });
  }
  ...
  castHeal(item: Item): void {
    ...
    if (this.player.hp === this.player.maxHp) {
      this.addMessage('You are already at full health.', CgaPalette.DARK_RED);
      return;
    }

    this.addMessage('Your wounds start to feel better!', CgaPalette.LIGHT_MAGENTA);
  ...
  castLightning(item: Item): void {
    ...
    if (!monster) {
      this.addMessage('No enemy is close enough to strike.', CgaPalette.LIGHT_RED);
      return;
    }
    ...
    this.addMessage(
      ...
      CgaPalette.LIGHT_BLUE
    );
    this.addMessage(`The damage is ${LIGHTNING_DAMAGE} hit points`, CgaPalette.LIGHT_BLUE);
  ...
  castFireball(item: Item): void {
    ...
    this.addMessage(
      ...
      CgaPalette.LIGHT_CYAN
    );
    this.startTargeting((x, y) => {
      if (this.player.distance(x, y) > FIREBALL_RANGE) {
        this.addMessage('Target out of range.', CgaPalette.LIGHT_GRAY);
        return;
      }
  ...
  castConfuse(item: Item): void {
    ...
    this.addMessage('Left-click to cast confuse, or right-click to cancel.', CgaPalette.LIGHT_CYAN);
    this.startTargeting((x, y) => {
      ...
      if (!monster) {
        this.addMessage('No monster there.', CgaPalette.LIGHT_GRAY);
        return;
      }
  ...
  renderAll(): void {
    ...
    if (!this.pathWalking && this.path) {
      for (let i = 1; i < this.path.length; i++) {
        term.drawChar(this.path[i].x, this.path[i].y, 0, 0, CgaPalette.WHITE);
      }
    }
    ...
    this.renderBar(
      ...
      CgaPalette.LIGHT_MAGENTA,
      CgaPalette.DARK_MAGENTA
    );

    term.drawString(1, PANEL_Y + 4, `Dungeon level ${this.level}`, CgaPalette.ORANGE);

    // Display names of objects under the mouse
    term.drawString(1, PANEL_Y, this.getNamesUnderMouse(), CgaPalette.LIGHT_GRAY);

    if (this.targetFunction) {
      const targetCell = term.getCell(this.targetCursor.x, this.targetCursor.y);
      if (targetCell) {
        targetCell.setBackground(CgaPalette.WHITE);
      }
    }
  ...
  nextLevel(): void {
    ...
    this.level++;
    this.addMessage('After a rare moment of peace, you descend deeper...', CgaPalette.LIGHT_RED);
    this.entities = [this.player];
