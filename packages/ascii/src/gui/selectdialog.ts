import { Key, Point, Rect, SimplePalette } from '@wglt/core';
...
export class SelectDialog extends Dialog {
  ...
  drawContents(console: Console, offset: Point): void {
    for (let i = 0; i < this.options.length; i++) {
      ...
      if (i === this.hoverIndex) {
        console.drawString(offset.x, offset.y + i, str, SimplePalette.BLACK, SimplePalette.WHITE);
      } else {
        console.drawString(offset.x, offset.y + i, str, SimplePalette.WHITE, SimplePalette.BLACK);
      }
    }
  }
  ...
  handleInput(terminal: Terminal, offset: Point): boolean {
    ...
    if (
      this.hoverIndex >= 0 &&
      (terminal.isKeyPressed(Key.VK_ENTER) || terminal.isKeyPressed(Key.VK_NUMPAD_ENTER))
    ) {
