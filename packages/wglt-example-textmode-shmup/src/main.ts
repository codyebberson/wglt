import { CgaPalette, fromRgb, Key, RNG, Terminal } from 'wglt';
...
const term = Terminal.init(WIDTH, HEIGHT);
...
term.update = (): void => {
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    const bullet = bullets[i];
    term.drawString(bullet.x, bullet.y, '*', CgaPalette.YELLOW);
    bullet.y -= 2;
    if (bullet.y < 0) {
      bullets.splice(i, 1);
    }
  ...
  for (let i = explosions.length - 1; i >= 0; i--) {
    ...
    term.drawString(Math.round(explosion.x), Math.round(explosion.y), '%', CgaPalette.ORANGE);
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    const enemy = enemies[i];
    term.drawString(enemy.x, enemy.y, '#', CgaPalette.LIGHT_GREEN);
    term.drawString(enemy.x - 1, enemy.y - 1, '@', CgaPalette.LIGHT_RED);
    term.drawString(enemy.x + 1, enemy.y - 1, '@', CgaPalette.LIGHT_RED);
    term.drawString(enemy.x - 1, enemy.y + 1, '@', CgaPalette.LIGHT_RED);
    term.drawString(enemy.x + 1, enemy.y + 1, '@', CgaPalette.LIGHT_RED);
  }

  term.drawString(player.x, player.y, '^', CgaPalette.LIGHT_GRAY);
  term.drawString(player.x - 1, player.y + 1, '/', CgaPalette.LIGHT_GRAY);
  term.drawString(player.x + 1, player.y + 1, '\\', CgaPalette.LIGHT_GRAY);
  term.drawString(player.x, player.y + 1, '|', CgaPalette.LIGHT_RED);

  if (waveTime < 0 && waveTime > -WAVE_WARMUP_TIME + 60) {
    term.drawCenteredString(WIDTH / 2, HEIGHT / 2, `WAVE ${wave.count}`, CgaPalette.WHITE);
  }

  term.drawString(1, HEIGHT - 2, 'Z - SHOOT', CgaPalette.WHITE);
